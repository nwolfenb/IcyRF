...
% inclusions embedded in a non-absorbing medium using the algorithm of
% Ulaby and Long (2014)
%
% Syntax:
...
%% Check if Host Medium is Absorbing
if abs(imag(epsb))>0
    error('Host medium must be non-absorbing (imag(epsb) == 0).')
end
...
lambda = c/f;
chi = (2*pi*r/lambda)*sqrt(epsb); % normalized circumference in background (8.31b)
